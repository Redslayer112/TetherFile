...
from transfers.wan.network import create_server_socket, get_local_ipv6
from transfers.wan.protocol import ACK_METADATA, DONE, FAIL, MISMATCH
...
def start_server(local_ip, port, ui, server_control):
	...
	try:
		...
		with ui_lock:
			...
			ui.print_colored(4, 2, f"🎯 Listening on port {port} (IPv4 + IPv6)", 'success')
			ipv6 = get_local_ipv6()
			if ipv6:
				ui.print_colored(5, 2, f"📡 Your IPv6: {ipv6}", 'highlight')
				ui.print_colored(6, 2, "   Share this with sender — direct P2P, no port forwarding!", 'info')
				ui.print_colored(8, 2, "💡 Waiting for sender...", 'highlight')
			else:
...
def recv_exact(sock, size, idle_timeout=None):
	"""Receive exactly `size` bytes; one timeout for the whole call."""
...
def _stream_recv_file(sock, file_obj, expected_size, transfer_timeout, progress, progress_offset=0):
	"""Stream `expected_size` bytes from sock into file_obj while hashing in
	one pass. Pre-allocated buffer + recv_into avoids per-chunk allocations
	and a single timeout is set for the whole transfer.
	"""
...
def dedupe_path(path):
	...
	Creates an empty placeholder so concurrent receivers cannot collide.
	"""
...
def receive_file(client_socket, file_info, ui, failed_validations, validation_lock, transfer_timeout):
	...
	if disk_usage:
		...
		if free_space < file_info['size'] * 1.1:
			with ui_lock:
				ui.show_message(
					f"❌ Insufficient disk space. Need {format_size(file_info['size'])}, have {format_size(free_space)}",
					'error'
				)
			raise OSError("Insufficient disk space for incoming file")

	safe_filename = sanitize_filename(file_info['name'])
	final_filepath = dedupe_path(os.path.join(RECEIVED_DIR, safe_filename))
	# dedupe_path created an empty placeholder; remove so shutil.move can replace it.
	try:
	...
	try:
		temp_fd, temp_filepath = tempfile.mkstemp(dir=RECEIVED_DIR, prefix=f".{safe_filename}_", suffix=".tmp")
		...
		try:
			dir_fd = os.open(RECEIVED_DIR, os.O_RDONLY)
			try:
				os.fsync(dir_fd)
			finally:
				os.close(dir_fd)
...
def receive_directory(client_socket, dir_info, ui, failed_validations, validation_lock, transfer_timeout):
	...
	if disk_usage:
		...
		if free_space < required_space * 1.1:
			with ui_lock:
				ui.show_message(
					f"Insufficient disk space. Required: {format_size(required_space)}, Available: {format_size(free_space)}",
					'error'
				)
		...
		for i, file_info in enumerate(dir_info['files'], 1):
			...
			try:
				...
				if actual_size != file_info['size']:
					raise Exception(
						f"Size mismatch for {safe_file_path}: expected {file_info['size']}, got {actual_size}"
					)
				...
				with ui_lock:
					ui.show_message(
						f"Error receiving {safe_file_path} (completed {files_completed}/{len(dir_info['files'])} files): {e}",
						'error'
					)
				raise
