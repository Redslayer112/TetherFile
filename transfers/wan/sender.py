...
from transfers.wan.network import create_socket, resolve_host
...
def _stream_send_file(sock, filepath, file_size, progress, sent_so_far=0):
    """Stream a file into sock while computing the configured hash in one pass.

    Pre-allocated bytearray + memoryview + readinto eliminates per-chunk
    Python bytes allocations (halves disk I/O versus pre-hash + send).
    Returns (digest_bytes, sent_total_after).
    """
...
def send_file(filepath, target_host, port, ui):
    ...
    ui.print_colored(5, 2, f"🎯 Target: {target_host}", 'info')

    sock = None
    try:
        candidates = resolve_host(target_host, port)
        ui.print_colored(7, 2, f"🔗 Connecting to {target_host}...", 'warning')
        ui.stdscr.refresh()
        for family, sockaddr in candidates:
            try:
                ...
                sock = None
        if sock is None:
            raise ConnectionRefusedError(f"Could not connect to {target_host}:{port}")
        sock.settimeout(max(CONNECTION_TIMEOUT, file_size // (1024 * 256)))

        ui.print_colored(8, 2, f"✅ Connected to receiver at {target_host}:{port}", 'success')
        ui.stdscr.refresh()

        # Streaming hash: digest computed during send and appended as a
        # raw trailer (no pre-flight disk pass).
        file_info = {
        ...
        with ui_lock:
            ui.show_message(f"🚫 Connection refused: Receiver might not be running on {target_host}:{port}", 'error')
        return False
    except socket.error as e:
        with ui_lock:
...
def send_directory(dir_path, target_host, port, ui):
    ...
    try:
        candidates = resolve_host(target_host, port)
        ui.print_colored(4, 2, f"Connecting to {target_host}:{port}...", 'warning')
        ui.stdscr.refresh()
        for family, sockaddr in candidates:
            try:
                sock = create_socket(family)
        ...
        for i, file_info in enumerate(files_info, 1):
            ui.print_colored(ui.height - 5, 2, f"[{i}/{len(files_info)}] {file_info['path']}", 'special')
            ui.stdscr.refresh()
